...
#參考https://github.com/jeknov/aiAlgorithmsWithPacman/blob/master/01.Search_BFS.DFS.UCS.Astar/search.py
def depthFirstSearch(problem: SearchProblem):
    ...
    #             dfs.push((child, actions+[direction], visited+[node]) )
...
#----------------------------------------------------------------------
    ...
    #getSuccessors會回傳 附近點、移動方向、step
    
    fringe = util.Stack()
    fringe.push( (problem.getStartState(), [], []) )

    while not fringe.isEmpty():
        # print("test:", fringe.list)
        node, actions, visited = fringe.pop()
        print("test : ", node, actions,)
        
        print("visited : ", node)

        if problem.isGoalState(node):
            # print("test ", problem.getSuccessors((2,1)))
            return path
        for child, direction, steps in problem.getSuccessors(node):
            # print("test : ", problem.getSuccessors(node))
            # print("child : ", child)
